...
class H5Message extends Base implements iMessageController
{
    ...
    {
        $log = new LogRequest('login', Category::CLIENT_TYPE_H5);
        ...
        $result = Result::getLoginResult(Category::CLIENT_TYPE_H5, $uid, $syncStamp);
    ...
    {
        $log = new LogRequest('logout', Category::CLIENT_TYPE_H5);
        $log->request($this->body);
        ...
        $result = Result::getLogoutResult(Category::CLIENT_TYPE_H5, $toUid, $syncStamp);
        $this->messageBase($result);
        return true;
    }
    ...
    {
        $log = new LogRequest('pull', Category::CLIENT_TYPE_H5);
        $log->request($this->body);
        ...
        //异步处理新增数据
        $model->addPull($noceAck, $toUid, $unread, Category::CLIENT_TYPE_H5);
    ...
    {
        $log = new LogRequest('read', Category::CLIENT_TYPE_H5);
        $log->request($this->body);
        ...
        if (0 == $count) {
            $result = Result::getReadResult(Category::CLIENT_TYPE_H5, $toUid, $syncStamp);
            $this->messageBase($result);
            return true;
        }
        ...
        $noceAck = $this->getNoceAck();
        $model->addRead($noceAck, $toUid, Category::CLIENT_TYPE_H5);
    ...
    {
        $log = new LogRequest('received', Category::CLIENT_TYPE_H5);
        $log->request($this->body);
        ...
        if ($count == 0) {
            $result = Result::getReceivedResult(Category::CLIENT_TYPE_H5, $noceAck, $syncStamp);
