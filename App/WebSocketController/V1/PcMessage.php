...
class PcMessage extends Base implements iMessageController
{
    ...
    {
        $log = new LogRequest('login', Category::CLIENT_TYPE_PC);
    ...
    {
        $log = new LogRequest('logout', Category::CLIENT_TYPE_PC);
        $log->request($this->body);
        ...
        $fdServer = new FdServer();
        $fd = $this->caller()->getClient()->getFd();
        $fUid = $fdServer->getSocketUid((string)$fd);
        ...
        $result = Result::getLogoutResult(Category::CLIENT_TYPE_PC, $toUid, $syncStamp);
        $this->messageBase($result);
        return true;
    }
    ...
    {
        $log = new LogRequest('pull', Category::CLIENT_TYPE_PC);
        $log->request($this->body);
        ...
        //异步处理新增数据
        $model->addPull($noceAck, $toUid, $unread, Category::CLIENT_TYPE_PC);
    ...
    {
        $log = new LogRequest('read', Category::CLIENT_TYPE_PC);
        $log->request($this->body);
        ...
        if (0 == $count) {
            $result = Result::getReadResult(Category::CLIENT_TYPE_PC, $toUid, $syncStamp);
            $this->messageBase($result);
            return true;
        }
        ...
        $noceAck = $this->getNoceAck();
        $model->addRead($noceAck, $toUid, Category::CLIENT_TYPE_PC);
    ...
    {
        $log = new LogRequest('received', Category::CLIENT_TYPE_PC);
        $log->request($this->body);
        ...
        if ($count == 0) {
            $result = Result::getReceivedResult(Category::CLIENT_TYPE_PC, $noceAck, $syncStamp);
