...
class IosMessage extends Base
{
    ...
    {
        $log = new LogRequest('setAuthor', Category::CLIENT_TYPE_IOS);
        ...
        $uid = $this->request->getToken($this->body['to_uid']);
        ...
        $result = Result::getSetAuthorResult(Category::CLIENT_TYPE_IOS, $uid, $syncStamp);
    ...
    {
        $log = new LogRequest('logoutAuthor', Category::CLIENT_TYPE_IOS);
        $log->request($this->body);
        ...
        $result = Result::getLogoutAuthorResult(Category::CLIENT_TYPE_IOS, $toUid, $syncStamp);
        $this->messageBase($result);
        return true;
    }
    ...
    {
        $log = new LogRequest('clearReadNumber', Category::CLIENT_TYPE_IOS);
        $log->request($this->body);
        ...
        $result = Result::unReadMessageResult(Category::CLIENT_TYPE_IOS, $syncStamp);
        $this->messageBase($result);
        return true;
    }
}
